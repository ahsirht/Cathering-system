...
const ServicePage = () => {
  ...
  const handleCancelService = async (serviceId) => {
    ...
    try {
      // Fetch the service to get the list of users
      onValue(serviceRef, async (snapshot) => {
        const service = snapshot.val()
        if (service && service.users) {
          // Set isInService to false for all users in this service
          for (let userId of service.users) {
            const userRef = ref(secondaryDb, `/users/${userId}`)
            await update(userRef, { isInService: false })
          }
        }
      ...
      notification.error({ message: "Error canceling service!" })
    }
  }
    ...
    onValue(
      ...
      (snapshot) => {
        ...
        if (data) {
          ...
          setUsers(
            userList.map((s) => ({ label: s.name, value: s.id })) // Use userId (Firebase generated ID) as the value
